...
namespace ArizaApp.Controllers
{
    public class NotificationController : BaseController
    {
        ...
        {
            ViewBag.planliCalismaNo = await DbContext.ArizaModels.Where(x => x.FaultType == "Planlı Çalışma").Distinct().OrderByDescending(x => x.FaultNo).Take(3).Select(x => x.FaultNo).ToListAsync();
            ViewBag.arizaNo = await DbContext.ArizaModels.Where(x => x.FaultType == "Arıza").Distinct().OrderByDescending(x => x.FaultNo).Take(3).Select(x => x.FaultNo).ToListAsync();
